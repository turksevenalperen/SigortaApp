...
import {
  Shield, CheckCircle, ChevronDown, TrendingUp, DollarSign, Headphones, Menu, X
} from 'lucide-react';
...
export default function HomePage({ onStart }: HomePageProps) {
  ...
  const [formData, setFormData] = useState({ name: '', phone: '', plate: '' });
  const [plateInputs, setPlateInputs] = useState({ city: '', letters: '', numbers: '' });
  ...
  const validatePlate = () => {
    ...
    if (!/^[A-Z]+$/.test(plateInputs.letters)) return 'Plaka harfleri sadece büyük harf içermelidir';
    if (!/^\d+$/.test(plateInputs.numbers)) return 'Plaka numarası sadece rakam içermelidir';
    if (plateInputs.numbers.length > 5) return 'Plaka numarası maksimum 5 haneli olabilir';
    return '';
  };
  ...
  const handleFormSubmit = async () => {
    ...
    if (nameError || phoneError || plateError) {
      setErrors({ name: nameError, phone: phoneError, plate: plateError });
      return;
    }
    ...
    try {
      const response = await fetch(`${API_URL}/api/cancel-request`, {
        ...
        body: JSON.stringify({ ...formData, plate: fullPlate })
      });
  ...
  const handleNavClick = (view: 'home' | 'campaigns' | 'cancel') => {
    setCurrentView(view);
    setMobileMenuOpen(false);
  };

  // ------ RESPONSIVE LOADING EKRANI ------
  if (!isAppLoaded) {
    return (
      <div className="min-h-screen flex flex-col justify-center items-center bg-linear-to-br from-blue-50 via-white to-slate-50">
        ...
        <div className="text-lg sm:text-xl text-blue-800 font-bold">Yükleniyor...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-linear-to-br from-blue-50 via-white to-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between h-16 sm:h-20">
            ...
            >
              ...
              <span className="text-lg sm:text-2xl font-bold text-slate-900">SigortaApp</span>
            </button>
            ...
            <div className="hidden md:flex items-center gap-6">
              <div className="relative">
                ...
                {productMenuOpen && (
                  <div className="absolute top-full left-0 mt-2 bg-white rounded-lg shadow-xl border border-slate-200 py-2 min-w-[180px] z-50">
          ...
          {mobileMenuOpen && (
            <div className="md:hidden border-t border-slate-200 py-4 space-y-2">
              <div>
                <button
                  onClick={() => setProductMenuOpen(!productMenuOpen)}
                  className="w-full flex items-center justify-between px-4 py-3 text-slate-700 hover:bg-slate-50 rounded-lg font-medium transition-colors"
                >
                  <span>Ürünlerimiz</span>
                  <ChevronDown className={`w-4 h-4 transition-transform ${productMenuOpen ? 'rotate-180' : ''}`} />
                </button>
                {productMenuOpen && (
                  <div className="mt-2 ml-4 space-y-1">
                    <button
                      onClick={() => handleProductClick('trafik')}
                      className="w-full text-left px-4 py-2 text-slate-600 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-colors"
                    >
                      Trafik Sigortası
                    </button>
                    <button
                    ...
                    <button
                      className="w-full text-left px-4 py-2 text-slate-400 rounded-lg"
                      aria-disabled="true"
                    >
                      Kasko (Yakında)
                    </button>
              ...
              <button
                onClick={() => handleNavClick('campaigns')}
                className="w-full text-left px-4 py-3 text-slate-700 hover:bg-slate-50 rounded-lg font-medium transition-colors"
              >
                Kampanyalar
              </button>
              <button
      ...
      {currentView === 'home' && (
        <div className="max-w-7xl mx-auto w-full px-2 sm:px-4 py-8">
          ...
          <div className="w-full max-w-4xl mx-auto mb-12 px-1 sm:px-0">
            <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-8 border border-slate-200">
              ...
              <div className="text-center">
                <button
                  onClick={onStart}
                  className="bg-blue-600 text-white px-8 sm:px-12 py-4 rounded-lg font-bold text-lg hover:bg-blue-700 transition-all transform hover:scale-105 shadow-lg hover:shadow-xl"
                >
                  Fiyat Öğren
                </button>
          ...
          <div className="w-full max-w-6xl mx-auto mb-12 px-1 sm:px-0">
